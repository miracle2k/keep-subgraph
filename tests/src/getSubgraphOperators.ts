...
export async function getAllSubgraphOperators(block: number) {
  const ops: OperatorsSubgraphResponse = await subgraphCall(
    `query { operators (first: 1000, block: {number: ${block}}) { owner address stakedAmount } }`
  );
...
export async function getCurrentSubgraphBlock() {
  const response: MetaSubgraphResponse = await subgraphCall(
